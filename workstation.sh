...
# Installs Docker, eksctl, kubectl, kubens, k9s
...
if ! command -v brew &>/dev/null; then
  ...
  if [[ -x "$BREW_PREFIX/bin/brew" ]]; then
    ...
    success "Homebrew installed and environment updated"
  else
    error "Homebrew install failed or not found at expected path"
  fi
else
